...
var NetworkingOverrideNetworkingPollCallback = ffi.Function('int', [ ToxContextPtr, 'uint32', 'pointer', 'pointer' ]);
...
var Tox = function(opts) {
  ...
  this._makesocket = opts.makesocket ? opts.makesocket : null;
  if (this._makesocket) {
  ...
  function packetpoll(ctx, sock, ip_port, dataPtr) {
    try {
      if (self._packets.length) {
        ...
        var rinfo = pkt[1];
        var ip_data = ref.reinterpret(ip_port, 24 + 2);
        var addr = ipaddr.parse(rinfo.address);
        var ipv4prefix = ipaddr.parse("::ffff:0:0:0:0");
        if (addr.match(ipv4prefix,32)) {
            var abytes = addr.toByteArray();
            ip_data[0] = 2;
            for (var i = 0; i < 4; i++) {
                ip_data[i+8] = abytes[i+12];
            }
        } else {
            var abytes = addr.toByteArray();
            ip_data[0] = 10;
        ...
        var dataPtr = ref.reinterpret(dataPtr, pkt[0].length);
        pkt[0].copy(dataPtr);
        return pkt[0].length;
      } else {
        return -1;
