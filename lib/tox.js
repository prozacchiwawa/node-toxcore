...
var IP_Port = RefArray('uint8', 4 + 16 + 2);
...
var NetworkingOverrideSendPacketCallback = ffi.Function('int', [ ToxContextPtr, 'uint32', ipPortPtr, RefArray('uint8', 4096), 'uint32' ]);
var NetworkingOverrideNetworkingPollCallback = ffi.Function('void', [ ToxContextPtr, 'uint32', ipPortPtr, RefArray('uint8', 4096), 'uint16' ]);
...
Tox.prototype._initNetCallbacks = function() {
  ...
  console.log('tox',this._instance);
  ...
  function sendpacket(ctx, sock, ip_port, data, length) {
    var args = {sock: sock - 3, length: length, data: data, ip_port: ip_port};
    console.log('sendpacket',args);
    self._emit('send', args);
    return 0;
  }

  function packetpoll(ctx, sock, af, packet) {
    console.log('packetpoll',sock);
  ...
  function newsocket(ctx, af, type, proto, addrbytes, addrlen, port) {
    console.log('newsocket',ctx);
    var sock = _sockets.create();
    try {
      console.log('newsocket',sock);
      self._emit('socket', { af: af, "type": type, proto: proto, addrbytes: addrbytes, addrlen: addrlen, port: port });
      console.log('return socket', sock+3);
      _sockets[sock] = { };
      return sock + 3;
    } catch (e) {
