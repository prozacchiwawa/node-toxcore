...
var ToxContextPtr = ref.refType(ref.types.void);

var IP_Port = RefArray('uint8', 8 + 16 + 2);
var ipPortPtr = ref.refType(IP_Port);

var Networking_PacketReceive = StructType({
    data: RefArray("uint8", 4096),
    length: 'uint32',
    port: 'uint16'
});
var Networking_PacketReceivePtr = ref.refType(Networking_PacketReceive);
...
var NetworkingOverrideSockValidCallback = ffi.Function('uint32', [ ToxContextPtr, 'uint32' ]);
var NetworkingOverrideKillSockCallback = ffi.Function('void', [ ToxContextPtr, 'uint32' ]);
var NetworkingOverrideNonBlockSockCallback = ffi.Function('int', [ ToxContextPtr, 'uint32' ]);
var NetworkingOverrideSetNoSigpipeCallback = ffi.Function('int', [ ToxContextPtr, 'uint32' ]);
var NetworkingOverrideSetReuseAddrCallback = ffi.Function('int', [ ToxContextPtr, 'uint32' ]);
var NetworkingOverrideSetDualStackCallback = ffi.Function('int', [ ToxContextPtr, 'uint32' ]);
var NetworkingOverrideSendPacketCallback = ffi.Function('int', [ ToxContextPtr, 'uint32', 'int', ipPortPtr, 'pointer', 'uint32' ]);
var NetworkingOverrideNetworkingPollCallback = ffi.Function('void', [ ToxContextPtr, 'uint32', Networking_PacketReceivePtr ]);
var NetworkingOverrideNewSocketCallback = ffi.Function('int', [ ToxContextPtr, 'uint32', 'uint32', 'uint32', ref.refType('string'), 'uint32', 'uint16' ]);

/** Represents an int-enumerated list with holes that are backfilled */
var LookasideList = function() {
  this._empty = [];
  this.length = 0;
}

LookasideList.prototype.create = function() {
  if (this._empty.length == 0) {
    ...
    var at = this._empty.shift();
    this[at] = null;
  }
}

LookasideList.prototype.destroy = function(n) {
  if (n >= 0 && n < this.length) {
    this[n] = null;
    this._empty.unshift(n);
  }
}

var _instances = new LookasideList();
var _sockets = new LookasideList();
...
var Tox = function(opts) {
  ...
  this._makesocket = opts.makesocket ? opts.makesocket : function() { };
  this._initNetCallbacks();
...
Tox.prototype.createLibrary = function(libpath) {
  ...
  return ffi.Library(libpath, {
    ...
    'tox_set_network_overrides': 
      [ 'void',
        [ ref.refType('void'),
          NetworkingOverrideSockValidCallback,
          NetworkingOverrideKillSockCallback,
          NetworkingOverrideNonBlockSockCallback,
          NetworkingOverrideSetNoSigpipeCallback,
          NetworkingOverrideSetReuseAddrCallback,
          NetworkingOverrideSetDualStackCallback,
          NetworkingOverrideSendPacketCallback,
          NetworkingOverrideNetworkingPollCallback,
          NetworkingOverrideNewSocketCallback
        ]
      ]
  });
};
...
Tox.prototype._initNetCallbacks = function() {
  this._instance = _instances.create();

  var self = this;
  function sock_valid(ctx,s) {
    console.log('sock_valid',s,_sockets);
    if (s >= 3 &&
        s < _sockets.length + 3) {
        console.log(s-3,_sockets);
        var res = !!_sockets[s - 3];
        console.log(s-3,res);
        return res;
    } else { return false; }
  }
    
  function kill_sock(ctx,s) {
    console.log('kill_sock',s);
    if (sock_valid(ctx,s)) {
      console.log('destroy',s-3);
      _sockets.destroy(s - 3);
      console.log('signal-destroy',s-3);
      self._emit('close', s - 3);
      console.log('done');
    }
  }
    
  function nonblock(ctx,s) {
      return true;
  }

  function nosigpipe(ctx, s) {
      return true;
  }
    
  function reuseaddr(ctx, s) {
      return true;
  }
    
  function dualstack(ctx, s) {
      return true;
  }

  function sendpacket(ctx, sock, af, ip_port, data, length) {
    var databuf = ref.reinterpret(data, length);
    var args = {sock: sock - 3, length: length, data: databuf, ip_port: ip_port};
    var printed = '';
    console.log('data',args.data);
    self._emit('send', args);
    return 0;
  }

  function packetpoll(ctx, sock, packet) {
    if (self._packets.length) {
      var pkt = self._packets.unshift();
      packet.data = pkt[0];
      packet.length = pkt[0].length;
      var rinfo = pkt[1];
      var ip_port_buf = new Buffer(8 + 16 + 4);
      ip_port_buf[0] = af;
      var abytes = rinfo.address.split('.');
      for (var i = 0; i < 4; i++) {
        ip_port_buf[i+8] = parseInt(abytes[i]);
      }
      ip_port_buf[24] = rinfo.port / 256;
      ip_port_buf[25] = rinfo.port & 255;
      packet.port = ip_port_buf;
      return 0;
    } else {
      return -1;
    }
  ...
  function newsocket(ctx, af, type, proto, addrbytes, addrlen, port) {
    var sock = _sockets.create();
    try {
      var req = { sock: sock, af: af, "type": type, proto: proto, addrbytes: addrbytes, addrlen: addrlen, port: port };
      _sockets[sock] = self._makesocket(req);
      return sock + 3;
    } catch (e) {
      console.log('error',e);
      _sockets.destroy(sock);
      return -1;
    }
  }

  this._sock_valid = this._toFFICallback(NetworkingOverrideSockValidCallback, sock_valid);
  this._kill_sock = this._toFFICallback(NetworkingOverrideKillSockCallback, kill_sock);
  this._nonblock = this._toFFICallback(NetworkingOverrideNonBlockSockCallback, nonblock);
  this._nosigpipe = this._toFFICallback(NetworkingOverrideSetNoSigpipeCallback, nosigpipe);
  this._reuseaddr = this._toFFICallback(NetworkingOverrideSetReuseAddrCallback, reuseaddr);
  this._dualstack = this._toFFICallback(NetworkingOverrideSetDualStackCallback, dualstack);
  this._sendpacket = this._toFFICallback(NetworkingOverrideSendPacketCallback, sendpacket);
  this._packetpoll = this._toFFICallback(NetworkingOverrideNetworkingPollCallback, packetpoll);
  this._newsocket = this._toFFICallback(NetworkingOverrideNewSocketCallback, newsocket);

  this._library.tox_set_network_overrides
    (new Buffer([]),
     this._sock_valid,
     this._kill_sock,
     this._nonblock,
...
Tox.prototype.receivedPacket = function(msg,rinfo) {
    this._packets.push([msg,rinfo]);
}
