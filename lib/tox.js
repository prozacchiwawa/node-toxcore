...
var NetworkingOverrideSendPacketCallback = ffi.Function('int', [ ToxContextPtr, 'uint32', 'int', 'pointer', 'pointer', 'uint32' ]);
var NetworkingOverrideNetworkingPollCallback = ffi.Function('int', [ ToxContextPtr, 'uint32', 'uint8*', 'uint8*' ]);
...
Tox.prototype._initNetCallbacks = function() {
  ...
  function sendpacket(ctx, sock, af, ip_port, data, length) {
    ...
    var ipbuf = ref.reinterpret(ip_port, 28);
    var args = {sock: sock - 3, length: length, data: Buffer.from(databuf), ip_port: Buffer.from(ipbuf)};
    var printed = '';
  ...
  function packetpoll(ctx, sock, ip_port, dataPtr) {
    try {
      if (self._packets.length) {
        var pkt = self._packets[0];
        self._packets = self._packets.slice(1);
        var data = pkt[0];
        var length = pkt[0].length;
        var rinfo = pkt[1];
        var ip_data = new Buffer(24 + 2);
        ip_data[0] = rinfo.family == 'IPv4' ? 2 : 10;
        var abytes = ipaddr.parse(rinfo.address).toByteArray();
        for (var i = 0; i < abytes.length; i++) {
          ip_data[i+8] = abytes[i];
        }
        ip_data[24] = rinfo.port / 256;
        ip_data[25] = rinfo.port & 255;
        ip_data.copy(ip_port, 0, 0, ip_data.length);
        data.copy(dataPtr, 0, 0, length);
        return length;
      } else {
        return -1;
      }
